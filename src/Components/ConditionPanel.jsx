...
export default function CalendarPanel() {
  ...
  return (
  <div className="flex-1 bg-white text-black px-4 py-6 sm:p-10 rounded-2xl h-full">
  <div className="w-full">
    <h2 className="text-xl sm:text-2xl font-bold mb-4 text-center">
      {monthName}, {year}
    </h2>

    <div className="grid grid-cols-7 gap-1 sm:gap-2 text-center text-gray-500 text-[10px] sm:text-sm mb-2 sm:mb-4 w-full">
      {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => (
        <div key={day}>{day}</div>
      ))}
    </div>

    <div className="grid grid-cols-7 gap-1 sm:gap-2 text-center text-[11px] sm:text-base w-full">
      {days.map((date, index) => (
        <div
          key={index}
